...
function Harpoon.loadAnimations( self )
...
function Harpoon.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets Harpoon renderable, change this to change the mesh
		self.tool:setFpRenderables( currentRenderablesFp )
		swapFpAnimation( self.fpAnimations, "unequip", "equip", 0.2 )
	end
end

function Harpoon.client_onUnequip( self, animate )
...
function Harpoon.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function Harpoon.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function Harpoon.onAim( self, aiming )
	self.aiming = aiming
...
function Harpoon.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function Harpoon.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function Harpoon.onShoot( self, dir )
...
function Harpoon.calculateFirePosition( self )
...
function Harpoon.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Harpoon.calculateFpMuzzlePos( self )
...
function Harpoon.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_harpoon, 1 ) then
			local firstPerson = self.tool:isInFirstPersonView()
...
function Harpoon.cl_onSecondaryUse( self, state )
...
function Harpoon.client_onEquippedUpdate( self, primaryState, secondaryState )
	--Raft
	if self.aimFireMode ~= nil then
		sm.gui.setProgressFraction(self.fireCooldownTimer/self.aimFireMode.fireCooldown)
	end
	--Raft
