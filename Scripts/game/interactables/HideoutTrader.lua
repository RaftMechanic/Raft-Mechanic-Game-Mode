...
function HideoutTrader.server_onFixedUpdate( self, timeStep )
	...
	if not QuestManager.Sv_IsQuestComplete("quest_deliver_vegetables") then
		if sm.container.canSpend(self.interactable:getContainer(), obj_crates_carrot, 2) and sm.container.canSpend(self.interactable:getContainer(), obj_crates_redbeet, 2) then
			QuestManager.Sv_OnEvent(QuestEvent.Vegetables)
		end
	elseif not QuestManager.Sv_IsQuestComplete("quest_deliver_fruits") then
		if sm.container.canSpend(self.interactable:getContainer(), obj_crates_blueberry, 2) and sm.container.canSpend(self.interactable:getContainer(), obj_crates_banana, 2) then
			QuestManager.Sv_OnEvent(QuestEvent.Fruits)
		end
	elseif not QuestManager.Sv_IsQuestComplete("quest_warehouse") then
		if sm.container.canSpend(self.interactable:getContainer(), obj_survivalobject_farmerball, 1) then
			QuestManager.Sv_OnEvent(QuestEvent.FarmerSuck)
		end
	end
end
...
function HideoutTrader.client_onInteract( self, character, state )
	...
	if state == true then
		if self.cl.user == nil then
			...
			--RAFT
			self.network:sendToServer("send_event")
		end
	end
end
...
function HideoutTrader.cl_selectAnimation( self )
	...
	--RAFT
	local unlocked = QuestManager.cl_getQuestProgressString(g_questManager, "quest_find_trader") == language_tag("Quest_FindTrader_TalkTrader") or QuestManager.Cl_IsQuestComplete("quest_find_trader")


	if self.cl.animations["Close"].isActive and self.cl.animations["Close"].playProgress >= 1.0 then
		if GetClosestPlayer( self.shape.worldPosition, OpenShutterDistance, self.shape.body:getWorld() ) ~= nil and unlocked then --RAFT
...
--RAFT
function HideoutTrader:client_canInteract()
	local unlocked = QuestManager.cl_getQuestProgressString(g_questManager, "quest_find_trader") == language_tag("Quest_FindTrader_TalkTrader") or QuestManager.Cl_IsQuestComplete("quest_find_trader")
...
function HideoutTrader:cl_msg(msg)
	sm.gui.chatMessage(language_tag("Trader") .. " " .. language_tag(msg))
end