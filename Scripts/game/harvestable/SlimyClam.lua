...
function SlimyClam.sv_onHit( self, player )
	if not self.harvested and sm.exists( self.harvestable ) then
		...
		if player and math.random() < 0.01 then
			self.network:sendToClient(player, "cl_divingMsg")
		end
...
function SlimyClam:cl_divingMsg()
	sm.gui.chatMessage(language_tag("ClamDivingSucks"))
end