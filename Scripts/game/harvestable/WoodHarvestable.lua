		...
		if self.treeParts[closestHitIdx].damage >= TrunkHealth then
			if self.data then
				if self.data.blueprint then
					...
					--RAFT spawn loot
					local loot = obj_leafytree_sapling
					local offset = 0.5
					if self.data.type == "small" then
						loot = obj_birchtree_sapling
					elseif self.data.type == "large" then
						loot = obj_pinetree_sapling
						offset = 1.5
					elseif self.data.crownHeight == 11 then
					...
					if quantity > 0 then
						...
						SpawnLoot( self.harvestable, lootList, self.harvestable.worldPosition + sm.vec3.new( math.random()*offset*2 - offset, math.random()*offset*2 - offset, 0.5 + offset ) )
					end
