	...
	if newPlayer then --Player is first time joiners
		...
		if player.id == 1 then
			if sm.json.fileExists("$SURVIVAL_DATA/LocalBlueprints/carried_raft.blueprint") then
				sm.creation.importFromFile(self.sv.saved.overworld, "$SURVIVAL_DATA/LocalBlueprints/carried_raft.blueprint",
					START_AREA_SPAWN_POINT)
				sm.log.info("[Raft Mechanic] No user made raft found, spawned default raft!")
			else
				sm.creation.importFromFile(self.sv.saved.overworld, "$CONTENT_DATA/LocalBlueprints/RAFT.blueprint",
					START_AREA_SPAWN_POINT)
