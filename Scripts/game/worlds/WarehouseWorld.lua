...
function WarehouseWorld.server_onCellCreated( self, x, y )
	...
	local loot_warehouserewardchest = {
		...
		--RAFT
		{ uuid = obj_interactive_propanetank_small, 		quantity = 3 },
	}
	...
	for _,chest in ipairs( chests ) do
		...
		for i,loot in ipairs( loot_warehouserewardchest ) do
			...
			sm.container.collect(container, loot.uuid, loot.quantity) --RAFT
		end
		sm.container.endTransaction()

		--RAFT
		sm.creation.importFromFile( self.world, "$CONTENT_DATA/LocalBlueprints/FARMER.blueprint", sm.vec3.new(97, 132, 20) )
	end
