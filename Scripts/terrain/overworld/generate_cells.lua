...
function generateOverworldCelldata( xMin, xMax, yMin, yMax, seed, data, padding )
	...
	pois[#pois + 1] = { x = -35, y = -30, type = POI_ROAD, rotation = 1, size = 1, road = false, flat = false, terrainType = TYPE_LAKE, cliffLevel = 0, cellDebug = DEBUG_R, edges = {} }
	...
	--guarantueed burn forest
	pois[#pois + 1] = { x = 2, y = 32, type = POI_BURNTFOREST_FARMBOTSCRAPYARD_LARGE, size = 4, road = false, flat = true, cellDebug = DEBUG_R, edges = {} }

	--Custom RAFT Mod Tiles
	pois[#pois + 1] = { x = 50, y = 42, type = POI_RAFT_WOCTEMPLE_XL, rotation = 0, size = 8, road = false, flat = true, cellDebug = DEBUG_R, edges = {} }
	...
	crashSiteExit.road = false
	mechanicStation.road = false
	packingStation1.road = false
	packingStation2.road = false
	...
	local convertToRoadPoi = function( poi, other )
		...
		if poi.rotation == nil and ( poi.type == POI_RAFT_RANGERSTATION_M or poi.type == POI_PACKINGSTATIONVEG_MEDIUM or poi.type == POI_PACKINGSTATIONFRUIT_MEDIUM ) then
		...
		if poi.type == POI_ROAD then
			g_cellTemp.road[poi.y][poi.x] = false
			if poi.rotation % 2 == 0 then -- W -> E
				g_cellTemp.road[poi.y][poi.x - 1] = false
				g_cellTemp.road[poi.y][poi.x + 1] = false
				if dx > 0 then
					return -1, 0
				else
				...
				g_cellTemp.road[poi.y - 1][poi.x] = false
				g_cellTemp.road[poi.y + 1][poi.x] = false
				if dy > 0 then
					return 0, -1
				else
		...
		if poi.type == POI_MECHANICSTATION_MEDIUM or poi.type == POI_PACKINGSTATIONVEG_MEDIUM or poi.type == POI_PACKINGSTATIONFRUIT_MEDIUM then
			if poi.rotation == 0 then
				g_cellTemp.road[poi.y - 1][poi.x] = false
				g_cellTemp.road[poi.y - 1][poi.x + 1] = false
				g_cellTemp.road[poi.y - 1][poi.x - 1] = false
				g_cellTemp.road[poi.y - 1][poi.x - 2] = false
				if dx > 0 then
					return -3, -1
				else
					return 2, -1
			...
			elseif poi.rotation == 1 then
				...
				g_cellTemp.road[poi.y + 1][poi.x] = false
				g_cellTemp.road[poi.y - 1][poi.x] = false
				g_cellTemp.road[poi.y - 2][poi.x] = false
				if dy > 0 then
					return 0, -3
				else
					return 0, 2
			...
			elseif poi.rotation == 2 then
				...
				g_cellTemp.road[poi.y][poi.x + 1] = false
				g_cellTemp.road[poi.y][poi.x - 1] = false
				g_cellTemp.road[poi.y][poi.x - 2] = false
				if dx > 0 then
					return -3, 0
				else
					return 2, 0
				...
				g_cellTemp.road[poi.y + 1][poi.x -1] = false
				g_cellTemp.road[poi.y - 1][poi.x -1] = false
				g_cellTemp.road[poi.y - 2][poi.x -1] = false
				if dy > 0 then
					return -1, -3
				else
					return -1, 2
		...
		if poi.type == POI_WAREHOUSE2_LARGE or poi.type == POI_WAREHOUSE3_LARGE or poi.type == POI_WAREHOUSE4_LARGE then
			if poi.rotation % 2 == 0 then
				...
				g_cellTemp.road[poi.y + 2][poi.x] = false
				g_cellTemp.road[poi.y - 2][poi.x - 1] = false
				g_cellTemp.road[poi.y - 3][poi.x - 1] = false
			else
				g_cellTemp.road[poi.y - 1][poi.x + 1] = false
				g_cellTemp.road[poi.y - 1][poi.x + 2] = false
				g_cellTemp.road[poi.y][poi.x - 2] = false
				g_cellTemp.road[poi.y][poi.x - 3] = false
			end
		...
		if poi.type == POI_RUINCITY_XL or poi.type == POI_SILODISTRICT_XL then
			-- E
			g_cellTemp.road[poi.y - 1][poi.x + 3] = false
			g_cellTemp.road[poi.y - 1][poi.x + 4] = false
			-- N
			g_cellTemp.road[poi.y + 3][poi.x] = false
			g_cellTemp.road[poi.y + 4][poi.x] = false
			-- W
			g_cellTemp.road[poi.y][poi.x - 4] = false
			g_cellTemp.road[poi.y][poi.x - 5] = false
			-- S
			g_cellTemp.road[poi.y - 4][poi.x - 1] = false
			g_cellTemp.road[poi.y - 5][poi.x - 1] = false
	...
	for _,edge in ipairs( edges ) do
		...
		for _,road in ipairs( roadCells ) do
			if insideCellBounds( road.x, road.y ) then
				g_cellTemp.road[road.y][road.x] = false
